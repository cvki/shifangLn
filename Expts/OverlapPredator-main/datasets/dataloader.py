...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    batched_points_list = []       # store src and tgt points
    batched_features_list = []      # store src and tgt feats
    batched_lengths_list = []       # store src and tgt lens
    assert len(list_data) == 1

    # IndoorDataset: getitem() return: src_pcd, tgt_pcd,  src_feats,  tgt_feats,   rot,   trans, correspondences,   src_pcd,   tgt_pcd,  torch.ones(1)
    #                               1.(13691,3), (14356,3),(13691,3),  (14356,3),  (3,3), (3,1),   (63277,2),       (13691,3), (14356,3), torch.ones(1)
    ...
    # concat src_pcd and tgt pcd (points,features,lengths) pt and feats ndim=2, lens ndim=1
    ...
    r_normal = config.first_subsampling_dl * config.conv_radius     # float #
    ...
    ###############
    dict_inputs = {
        'points': input_points,     # kpconv的四层结构中，每层的输入点：(28047,3),(7146,3),2057,3),(589,3)
    ...
    return dict_inputs      # 在KPConv中网络各层的参数和其他数据等。
