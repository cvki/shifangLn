...
def to_tsfm(rot,trans):     # combine 3*3 trans_mat and 3*1 trans_vec to 4*4 trans_mat
    tsfm = np.eye(4)
    tsfm[:3,:3]=rot
    tsfm[:3,3]=trans.flatten()
    return tsfm
    
def to_o3d_pcd(xyz):    # type_trans: arraylike/tensor to pcd
    """
...
def get_correspondences(src_pcd, tgt_pcd, trans, search_voxel_size, K=None):    # subsamling first K points
    src_pcd.transform(trans)    # affine transformation， trans--rigid 4*4 mat
    pcd_tree = o3d.geometry.KDTreeFlann(tgt_pcd)    # create tgt_pcd kdTree

    correspondences = []
    for i, point in enumerate(src_pcd.points):
        [count, idx, _] = pcd_tree.search_radius_vector_3d(point, search_voxel_size)    # 查找距离定位点小于给定半径的所有点, kdTree for efficiency
        if K is not None:
            idx = idx[:K]
        for j in idx:
            correspondences.append([i, j])      # i:src_point_idx, j:tgt_point_idx
