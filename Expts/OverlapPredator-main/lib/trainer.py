...
class Trainer(object):
    ...
    def inference_one_batch(self, inputs, phase):
        ...
        if(phase == 'train'):
            ...
            with torch.no_grad():
                ...
                # pt1_infered=o3d.geometry.PointCloud()
                # pt2_infered=o3d.geometry.PointCloud()
    ...
    def inference_one_epoch(self,epoch, phase):
        ...
        # init stats meter
        stats_meter = self.stats_meter()       # init meters

        num_iter = int(len(self.loader[phase].dataset) // self.loader[phase].batch_size)      # iteration counts
        ...
        for c_iter in tqdm(range(num_iter)): # loop through this epoch   
            ...
            inputs = c_loader_iter.next()    # it will call __len__() and __getitem__()
