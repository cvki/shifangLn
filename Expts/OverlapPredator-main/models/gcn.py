...
class SelfAttention(nn.Module):
    ...
    def forward(self, coords, features):    # self-attention,inception
        """
    ...
    def forward(self, coords0, coords1, desc0, desc1):
        for layer, name in zip(self.layers, self.names):
            if name == 'cross':
                ...
                desc0 = desc0 + layer(desc0, desc1)     # merge feats1 to feats0
                desc1 = desc1 + layer(desc1, desc0)     # merge feats0 to feats1
            elif name == 'self':
                desc0 = layer(coords0, desc0)       # merge neighborhoods feats to superpoints
