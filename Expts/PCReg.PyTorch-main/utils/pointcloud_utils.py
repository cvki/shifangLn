...
class PointcloudRotate(object):
    def __call__(self, points):
        ...
        points[:, 0:3] = torch.matmul(points[:, 0:3].to(torch.float32), rotation_matrix.t())
        return np.array(points[:,0:3]),np.array(rotation_matrix)
...
class PointcloudTranslate(object):
    # def __init__(self, translate_range=0.1, p=0.99):
...
class PointcloudRandomCropTwice(object):
    # def __init__(self, x_min=0.6, x_max=1.1, ar_min=0.75, ar_max=1.33, min_num_points=4096, max_try_num=10):
    def __init__(self, x_min=0.6, x_max=1.1, ar_min=0.75, ar_max=1.33):
        self.x_min = x_min
        self.x_max = x_max
    ...
    def __call__(self, points):
        ...
        while count_pts<2:
            ...
            if(np.shape(new_points)[0]>5500 and np.shape(new_points)[0]<6500):
