...
class Benchmark(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.in_dim1 == 3:
            ...
            transformed_pts = batch_transform(x.permute(0, 2, 1)[:, :, :3].contiguous(), batch_R, batch_t)
            transformed_nls = batch_transform(x.permute(0, 2, 1)[:, :, 3:].contiguous(), batch_R)
            transformed_x = torch.cat([transformed_pts, transformed_nls], dim=-1)
        else:
            raise ValueError
        return batch_R, batch_t, transformed_x      # return batch R,t,res_trans_xPointCloud
...
class IterativeBenchmark(nn.Module):
    ...
    def forward(self, x, y):
        ...
        for i in range(self.niters):
            batch_R, batch_t, transformed_x = self.benckmark(transformed_x, y)      # return batch个 R，T，transX
            transformed_xs.append(transformed_x)
            batch_R_res = torch.matmul(batch_R, batch_R_res)        # iteration R,t
