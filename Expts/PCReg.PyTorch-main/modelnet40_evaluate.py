    ...
    with torch.no_grad():
        for i, (ref_cloud, src_cloud, gtR, gtt) in tqdm(enumerate(test_loader)):
            ...
            R, t, pred_ref_cloud = model(src_cloud.permute(0, 2, 1).contiguous(), 
                    ref_cloud.permute(0, 2, 1).contiguous())    # R:3x3, t:3*1, pre_ref_cloud:
            toc = time.time()
            dura.append(toc - tic)      # record validation model running time
...
if __name__ == '__main__':
    ...
    args = config_params()
    print(args)
    test_set = MyModelNet40(root=args.root,
                          # npts=args.infer_npts,
                          train=False,
                          normal=args.normal,
                          mode=args.mode)   # valid
