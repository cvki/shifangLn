...
class MyModelnet40(Dataset):
    def __init__(self, args,count_pcs=None,num_pts=None) -> None:
        self.path = os.listdir(os.path.join(args.root, args.mode))
    ...
    def __getitem__(self, index):
        ...
        pts1, pts2, R, t, qutanr = datas['pts1'], datas['pts2'], datas['trans_mat'], datas['trans_vec'], datas[
            'quat_vec']
        if not self.num_pts is None:  # when there're too many datas in a pickle: sampling
            # num_pts should be less than num of points in a file
    ...
    def __len__(self):  # when there're too many pickle files in a dir: sampling. Bug:only use the top count_pcs files
        return len(self.filenames) if self.count_pcs is None or self.count_pcs > len(self.filenames) else self.count_pcs
