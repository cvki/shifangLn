...
from testAPI import visualizepc2
...
class Evaluator(nn.Module):
    ...
    def evaluate_fine(self, output_dict, data_dict):
        ...
        src_corr_points = apply_transform(src_corr_points, transform.squeeze())
    ...
    def evaluate_registration(self, output_dict, data_dict):
        ...
        src_points = output_dict['src_points'].squeeze()

        rre, rte = isotropic_transform_error(transform, est_transform)

        realignment_transform = torch.matmul(torch.inverse(transform), est_transform).squeeze()
        realigned_src_points_f = apply_transform(src_points, realignment_transform).squeeze()
        rmse = torch.linalg.norm(realigned_src_points_f - src_points, dim=1).mean()
        recall = torch.lt(rmse, self.acceptance_rmse).float()

        # visualizepc2(realigned_src_points_f.cpu(), src_points.cpu(),'gt-src, est-src')
