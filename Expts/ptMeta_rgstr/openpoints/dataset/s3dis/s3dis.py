...
class S3DIS(Dataset):
    ...
    """
    def __init__(self,
                 data_root: str = '/d2/code/datas/S3DIS',
                 test_area: int = 5,
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            cdata = np.load(data_path).astype(np.float32)
            # print('\n')
            # print(np.min(cdata[:,3]),'\t',np.max(cdata[:,3]))     # color RGB
            # print(np.min(cdata[:,4]),'\t',np.max(cdata[:,4]))     # color RGB
            # print(np.min(cdata[:,5]),'\t',np.max(cdata[:,5]))     # color RGB
            # print(np.min(cdata[:,6]),'\t',np.max(cdata[:,6]))     # label classID
            cdata[:, :3] -= np.min(cdata[:, :3], 0)
        ...
        if 'heights' not in data.keys():
            data['heights'] =  torch.from_numpy(coord[:, self.gravity_dim:self.gravity_dim+1].astype(np.float32))   # coord-y before transform
        return data
