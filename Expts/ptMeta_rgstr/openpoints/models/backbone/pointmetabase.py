...
class PointMetaBaseEncoder(nn.Module):
    ...
    def forward_seg_feat(self, p0, f0=None):
        # if hasattr(p0, 'keys'):
        #     # points=torch.cat((p0['ref_points'],p0['src_points']),dim=1)
        #     # feats=torch.cat((p0['ref_feats'],p0['src_feats']),dim=1).transpose(1, 2).contiguous()
        #     points = p0['points'].unsqueeze(0)
        #     feats = p0['features'].transpose(1,2).contiguous().unsqueeze(0)
        #     # p0, f0 = p0['pos'], p0.get('x', None)   # p0:xyz (N,3),  f0:rgb,untrans-y (N,4)
        # # if f0 is not None:
        # points = p0.unsqueeze(0)
        # feats = f0.transpose(0, 1).contiguous().unsqueeze(0)
        # if feats is None:
        #     feats = points.clone().transpose(1, 2).contiguous()
        points = p0
        feats = f0.transpose(1, 2).contiguous()
        p, f = [points], [feats]
        for i in range(0, len(self.encoder)):
            if i == 0:
                pe = None
